...
var Base = Class.extend({
    ...
    _checkStatusForError: function(statusCode, body) {
        if (statusCode === 401) {
            ...
            return (function(){
                var message = (body && body.error && body.error.type) ? body.error.type : 'UNPROCESSABLE_ENTITY';
                var type = (body && body.error && body.error.message) ? body.error.message : 'The operation cannot be processed';
